...
{
    /// <summary>
    /// "근접 무기로만 모더 처치" 특수 퀘스트(Mountain_Quest5) 조건 추적.
    /// 로컬 플레이어가 모더(Dragon)를 처음 타격해 전투가 시작된 시점부터 처치까지
    /// 원거리 무기(활/석궁) 또는 마법(원소/피 마법)을 사용했는지 기록한다.
    /// QuestGdKingMeleeOnlyTracker와 동일한 구조이며 대상 프리팹만 다르다.
    /// </summary>
    [HarmonyPatch]
    public static class QuestModerMeleeOnlyTracker
    {
        ...
        public static void OnDamage(Character __instance, HitData hit)
        {
            ...
            {
                Plugin.Log.LogError($"[QuestModerMeleeOnlyTracker] 오류: {ex.Message}");
            }
        }
