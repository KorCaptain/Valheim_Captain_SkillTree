    ...
    public static class Paladin_Config
    {
        ...
        public static string GetPaladinTooltip()
        {
            try
            {
                ...
                // 설명 섹션
                tooltip += $"<color=#90EE90><size=16>설명: </size></color><color=#E0E0E0><size=16>범위 아군 체력 {allyHealPercent}%/{interval}초 ({duration}초), 자가 {selfHealPercent}% 즉시</size></color>\n";
                ...
                tooltip += $"<color=#F0E68C><size=16>⚠️확인사항: </size></color><color=#FFE4B5><size=16>{confirmation}</size></color>\n";

                // 필요 아이템 섹션
                string requiredItem = "에이크쉬르 트로피";
                tooltip += $"<color=#87CEEB><size=16>⚠️필요 아이템: </size></color><color=#FF6B6B><size=16>{requiredItem}</size></color>";
