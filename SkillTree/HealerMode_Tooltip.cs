using System;
using CaptainSkillTree.Localization;

namespace CaptainSkillTree.SkillTree
{
    public static class HealerMode_Tooltip
    {
        public static string GetHealerModeTooltip()
        {
            Plugin.Log.LogDebug("[힐러모드 툴팁] GetHealerModeTooltip() 호출됨");

            try
            {
                int currentLevel = SkillTreeManager.Instance?.GetSkillLevel("staff_Step6_heal") ?? 0;
                int mainLevel    = currentLevel == 0 ? 1 : currentLevel;
                int displayLevel = Math.Min(currentLevel + 1, 7);

                float mainHeal      = SkillEffect.GetStaffHealPercentForLevel(mainLevel);
                float mainCd        = SkillEffect.GetStaffHealCooldownForLevel(mainLevel);
                float mainLightning = SkillEffect.GetLightningDamagePercent(mainLevel);
                float eitr          = Staff_Config.StaffHealEitrCostValue;
                float range         = Staff_Config.StaffHealRangeValue;
                int   pts           = Staff_Config.StaffHealRequiredPointsValue;

                var t = $"<color=#FFD700><size=22>{L.Get("staff_skill_heal")}</size></color>\n";
                t += $"<color=#E0E0E0><size=16>Lv{mainLevel} : {L.Get("healer_lv_stat", (int)mainHeal, (int)mainCd)}</size></color>\n";
                t += $"<color=#FFFF00><size=16>⚡ {L.Get("staff_heal_lightning_desc", (int)mainLightning)}</size></color>\n";
                t += $"<color=#87CEEB><size=16>{L.Get("tooltip_range")}: </size></color>";
                t += $"<color=#B0E0E6><size=16>{range:F0}m</size></color>\n";
                t += $"<color=#FFB347><size=16>{L.Get("tooltip_cost")}: </size></color>";
                t += $"<color=#FFDAB9><size=16>{L.Get("stat_eitr")} {eitr:F0}</size></color>\n";
                t += $"<color=#FF1493><size=16>{L.Get("tooltip_skill_type")}: </size></color>";
                t += $"<color=#00FFFF><size=16>{L.Get("skill_type_active_h")}</size></color>\n";
                t += $"<color=#FFA500><size=16>{L.Get("tooltip_cooldown")}: </size></color>";
                t += $"<color=#FFDB58><size=16>{mainCd:F0}{L.Get("unit_seconds")}</size></color>\n";
                t += $"<color=#98FB98><size=16>{L.Get("tooltip_requirements")}: </size></color>";
                t += $"<color=#00FF00><size=16>{L.Get("requirement_staff_wand")}</size></color>\n";
                t += $"<color=#F0E68C><size=16>{L.Get("tooltip_notice")}: </size></color>";
                t += $"<color=#FFE4B5><size=16>{L.Get("healer_self_heal_excluded")}</size></color>\n";
                t += $"<color=#87CEEB><size=16>{L.Get("tooltip_required_points")}: </size></color>";
                t += $"<color=#FF6B6B><size=16>{pts}</size></color>\n";

                if (currentLevel < 7)
                {
                    t += $"<color=#FFA500><size=16>{L.Get("staff_heal_next_level_req", displayLevel)}: </size></color>";
                    t += $"<color=#FF6B6B><size=16>{GetHealLevelItemText(displayLevel)}</size></color>\n";
                }
                else
                {
                    t += $"<color=#FFD700><size=16>{L.Get("staff_heal_max_level")}</size></color>\n";
                }

                if (mainLevel < 7)
                {
                    t += $"<color=#808080><size=14>────────────────────────────────────</size></color>\n";
                    for (int lv = mainLevel + 1; lv <= 7; lv++)
                    {
                        float h   = SkillEffect.GetStaffHealPercentForLevel(lv);
                        float cd  = SkillEffect.GetStaffHealCooldownForLevel(lv);
                        float ltg = SkillEffect.GetLightningDamagePercent(lv);
                        t += $"<color=#808080><size=14>Lv{lv} : {L.Get("healer_lv_stat", (int)h, (int)cd)} ⚡{(int)ltg}%</size></color>\n";
                    }
                }

                return t.TrimEnd('\n');
            }
            catch (Exception ex)
            {
                Plugin.Log.LogError($"[힐러모드 툴팁] 툴팁 생성 실패: {ex.Message}");
                return L.Get("tooltip_generation_error");
            }
        }

        private static string GetHealLevelItemText(int targetLevel)
        {
            switch (targetLevel)
            {
                case 1: return L.Get("item_trophy_eikthyr") + " x1 + " + L.Get("item_trophy_boar") + " x1";
                case 2: return L.Get("item_trophy_elder") + " x1 + " + L.Get("item_trophy_troll") + " x1";
                case 3: return L.Get("item_trophy_bonemass") + " x1 + " + L.Get("item_trophy_abomination") + " x1";
                case 4: return L.Get("item_trophy_dragonqueen") + " x1 + " + L.Get("item_trophy_sgolem") + " x1";
                case 5: return L.Get("item_trophy_goblinking") + " x1 + " + L.Get("item_trophy_goblinshaman") + " x1";
                case 6: return L.Get("item_trophy_seekerqueen") + " x1 + " + L.Get("item_trophy_gjall") + " x1";
                case 7: return L.Get("item_trophy_fader") + " x1 + " + L.Get("item_trophy_fallenvalkyrie") + " x1";
                default: return "";
            }
        }

        public static void UpdateHealerModeTooltip()
        {
            try
            {
                var manager = SkillTreeManager.Instance;
                if (manager?.SkillNodes != null && manager.SkillNodes.ContainsKey("staff_Step6_heal"))
                {
                    manager.SkillNodes["staff_Step6_heal"].Description = GetHealerModeTooltip();
                }
                else
                {
                    Plugin.Log.LogWarning("[힐러모드 툴팁] staff_Step6_heal 스킬 노드를 찾을 수 없음");
                }
            }
            catch (Exception ex)
            {
                Plugin.Log.LogError($"[힐러모드 툴팁] 업데이트 실패: {ex.Message}");
            }
        }
    }
}
