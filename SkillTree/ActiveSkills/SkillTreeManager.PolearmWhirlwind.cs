...
{
    // 폴암 휠윈드(polearm_step6_whirlwind) 레벨 시스템
    // Lv1: 에이크쉬르 + 멧돼지 트로피 / Lv2~7: 보스 트로피 + 바이옴 트로피
    public partial class SkillTreeManager
    {
        public bool HasPolearmWhirlwindLevelItems(int targetLevel)
        {
        ...
        public List<string> GetMissingPolearmWhirlwindItems(int targetLevel)
        {
            var player = Player.m_localPlayer;
        ...
        private void ConsumePolearmWhirlwindLevelItems(int targetLevel)
        {
            var player = Player.m_localPlayer;
