...
{
    // 단 한 발(crossbow_Step6_expert) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckOneShotInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0
                && SkillTree.SkillTreeManager.Instance.GetSkillLevel("crossbow_Step5_final") <= 0
                && SkillTree.SkillTreeManager.Instance.GetSkillLevel("crossbow_Step4_rapid_fire_lv2") <= 0)
                return new InvestResult(false, L10n.Get("oneshot_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasOneShotLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingOneShotItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleOneShotClick(SkillTree.SkillNode node)
        {
            if (node.Id != "crossbow_Step6_expert") return false;

            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("crossbow_Step6_expert");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Crossbow_Config.CrossbowOneShotRequiredPointsValue;
            ...
            if (isAdmin || manager.HasOneShotLevelItems(targetLevel))
            {
                ...
                ShowOneShotUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
