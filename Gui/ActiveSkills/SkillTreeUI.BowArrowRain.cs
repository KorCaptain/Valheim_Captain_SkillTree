...
{
    // 화살비(bow_Step6_arrow_rain) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckArrowRainInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("bow_Step5_master") <= 0)
                return new InvestResult(false, L10n.Get("arrow_rain_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasArrowRainLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingArrowRainItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleArrowRainClick(SkillTree.SkillNode node)
        {
            if (node.Id != "bow_Step6_arrow_rain") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("bow_Step6_arrow_rain");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Bow_Config.ArrowRainRequiredPointsValue;
            ...
            if (isAdmin || manager.HasArrowRainLevelItems(targetLevel))
            {
                ...
                ShowArrowRainUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
