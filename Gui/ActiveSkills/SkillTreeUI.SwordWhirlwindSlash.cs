...
{
    // 회오리베기(sword_step5_defswitch) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckWhirlwindSlashInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("sword_step4_duel") <= 0)
                return new InvestResult(false, L10n.Get("whirlwind_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasWhirlwindSlashLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingWhirlwindSlashItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleWhirlwindSlashClick(SkillTree.SkillNode node)
        {
            if (node.Id != "sword_step5_defswitch") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("sword_step5_defswitch");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Sword_Config.SwordStep5DefenseSwitchRequiredPointsValue;
            ...
            if (isAdmin || manager.HasWhirlwindSlashLevelItems(targetLevel))
            {
                ...
                ShowWhirlwindSlashUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
                var missing = manager.GetMissingWhirlwindSlashItems(targetLevel);
                var msg = L10n.Get("whirlwind_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("whirlwind_missing_items", string.Join(", ", missing));
        ...
        // ─── 업그레이드 확인 다이얼로그 ───
        private void ShowWhirlwindSlashUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            if (confirmDialog != null)
                DestroyImmediate(confirmDialog);

            confirmDialog = new GameObject("WhirlwindSlashUpgradeDialog");
