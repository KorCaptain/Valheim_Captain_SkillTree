...
namespace CaptainSkillTree.Gui
{
    ...
    public partial class SkillTreeUI
    {
        ...
        private InvestResult CheckHealInvest(SkillTree.SkillNode node, int currentLevel)
        {
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasHealLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingHealItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleHealClick(SkillTree.SkillNode node)
        {
            ...
            if (isAdmin || manager.HasHealLevelItems(targetLevel))
            {
                ...
                ShowHealUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowHealUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            borderImage.color = new Color(0.1f, 0.6f, 0.3f, 0.85f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.05f, 0.25f, 0.12f, 1f);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.10f, 0.45f, 0.20f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
