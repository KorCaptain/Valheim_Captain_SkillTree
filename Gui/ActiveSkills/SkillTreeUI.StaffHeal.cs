...
{
    // 힐(staff_Step6_heal) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckStaffHealInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("staff_Step5_archmage") <= 0)
                return new InvestResult(false, L10n.Get("staff_heal_prereq_required"));
            int targetLevel = currentLevel + 1;
            if (targetLevel > 7)
                return new InvestResult(false, L10n.Get("staff_heal_max_level"));
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasStaffHealLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingStaffHealItems(targetLevel);
                var msg = L10n.Get("staff_heal_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("staff_heal_missing_items", string.Join(", ", missing));
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleStaffHealClick(SkillTree.SkillNode node)
        {
            if (node.Id != "staff_Step6_heal") return false;

            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("staff_Step6_heal");
            int targetLevel = skillLevel + 1;

            if (targetLevel > 7)
            {
                tooltipUI.ShowWarning(L10n.Get("staff_heal_max_level"));
                return true;
            }
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Staff_Config.StaffHealRequiredPointsValue;
            ...
            if (isAdmin || manager.HasStaffHealLevelItems(targetLevel))
            {
                ...
                ShowStaffHealUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
                var missing = manager.GetMissingStaffHealItems(targetLevel);
                var msg = L10n.Get("staff_heal_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("staff_heal_missing_items", string.Join(", ", missing));
        ...
        // ─── 업그레이드 확인 다이얼로그 ───
        private void ShowStaffHealUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            if (confirmDialog != null)
                DestroyImmediate(confirmDialog);

            confirmDialog = new GameObject("StaffHealUpgradeDialog");
            ...
            darkBgImage.color = new Color(0.05f, 0.12f, 0.06f, 0.97f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.05f, 0.25f, 0.08f, 1f);
            ...
            titleText.text = L10n.Get("staff_heal_upgrade_title");
            titleText.fontSize = 20;
            titleText.fontStyle = FontStyle.Bold;
            titleText.color = new Color(0.20f, 0.90f, 0.40f, 1f);
            ...
            contentText.text = L10n.Get("staff_heal_upgrade_confirm", targetLevel);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.10f, 0.45f, 0.15f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
