...
namespace CaptainSkillTree.Gui
{
    ...
    public partial class SkillTreeUI
    {
        ...
        private InvestResult CheckWhirlwindInvest(SkillTree.SkillNode node, int currentLevel)
        {
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasWhirlwindLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingWhirlwindItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleWhirlwindClick(SkillTree.SkillNode node)
        {
            ...
            if (isAdmin || manager.HasWhirlwindLevelItems(targetLevel))
            {
                ...
                ShowWhirlwindUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
