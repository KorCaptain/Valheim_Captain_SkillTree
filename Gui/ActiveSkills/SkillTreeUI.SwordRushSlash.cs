...
{
    // 돌진베기(sword_step5_finalcut) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckRushSlashInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("sword_step4_duel") <= 0)
                return new InvestResult(false, L10n.Get("rush_slash_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasRushSlashLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingRushSlashItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleRushSlashClick(SkillTree.SkillNode node)
        {
            if (node.Id != "sword_step5_finalcut") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("sword_step5_finalcut");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Sword_Config.RushSlashRequiredPointsValue;
            ...
            if (isAdmin || manager.HasRushSlashLevelItems(targetLevel))
            {
                ...
                ShowRushSlashUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowRushSlashUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            borderImage.color = new Color(0.05f, 0.15f, 0.25f, 0.85f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.05f, 0.15f, 0.3f, 1f);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.05f, 0.25f, 0.5f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
