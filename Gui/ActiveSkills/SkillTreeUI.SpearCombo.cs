...
{
    // 연공창(spear_Step5_combo) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckSpearComboInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("spear_Step4_triple") <= 0)
                return new InvestResult(false, L10n.Get("spear_combo_prereq_required"));
            int targetLevel = currentLevel + 1;
            if (targetLevel > 7)
                return new InvestResult(false, L10n.Get("spear_combo_max_level"));
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasSpearComboLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingSpearComboItems(targetLevel);
                var msg = L10n.Get("spear_combo_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("spear_combo_missing_items", string.Join(", ", missing));
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleSpearComboClick(SkillTree.SkillNode node)
        {
            if (node.Id != "spear_Step5_combo") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("spear_Step5_combo");
            int targetLevel = skillLevel + 1;
            if (targetLevel > 7)
            {
                tooltipUI.ShowWarning(L10n.Get("spear_combo_max_level"));
                return true;
            }
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Spear_Config.SpearComboRequiredPointsValue;
            ...
            if (isAdmin || manager.HasSpearComboLevelItems(targetLevel))
            {
                ...
                ShowSpearComboUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
                var missing = manager.GetMissingSpearComboItems(targetLevel);
                var msg = L10n.Get("spear_combo_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("spear_combo_missing_items", string.Join(", ", missing));
        ...
        // ─── 업그레이드 확인 다이얼로그 ───
        private void ShowSpearComboUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            if (confirmDialog != null)
                DestroyImmediate(confirmDialog);

            confirmDialog = new GameObject("SpearComboUpgradeDialog");
            ...
            titleText.text = L10n.Get("spear_combo_upgrade_title");
            ...
            contentText.text = L10n.Get("spear_combo_upgrade_confirm", targetLevel);
