...
namespace CaptainSkillTree.Gui
{
    ...
    public partial class SkillTreeUI
    {
        ...
        private InvestResult CheckComboInvest(SkillTree.SkillNode node, int currentLevel)
        {
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasComboItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingComboItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleComboClick(SkillTree.SkillNode node)
        {
            ...
            if (isAdmin || manager.HasComboItems(targetLevel))
            {
                ...
                ShowComboUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
