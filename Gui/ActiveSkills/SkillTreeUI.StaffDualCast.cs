...
{
    // 이중시전(staff_Step6_dual_cast) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckDualCastInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("staff_Step5_archmage") <= 0)
                return new InvestResult(false, L10n.Get("dual_cast_prereq_required"));
            int targetLevel = currentLevel + 1;
            if (targetLevel > 7)
                return new InvestResult(false, L10n.Get("dual_cast_max_level"));
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasDualCastLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingDualCastItems(targetLevel);
                var msg = L10n.Get("dual_cast_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("dual_cast_missing_items", string.Join(", ", missing));
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleDualCastClick(SkillTree.SkillNode node)
        {
            if (node.Id != "staff_Step6_dual_cast") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("staff_Step6_dual_cast");
            int targetLevel = skillLevel + 1;
            if (targetLevel > 7)
            {
                tooltipUI.ShowWarning(L10n.Get("dual_cast_max_level"));
                return true;
            }
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            ...
            if (isAdmin || manager.HasDualCastLevelItems(targetLevel))
            {
                ...
                ShowDualCastUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowDualCastUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            borderImage.color = new Color(0.15f, 0.10f, 0.50f, 0.85f);
            ...
            darkBgImage.color = new Color(0.06f, 0.05f, 0.20f, 0.97f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.08f, 0.05f, 0.30f, 1f);
            ...
            titleText.text = L10n.Get("dual_cast_upgrade_title");
            titleText.fontSize = 20;
            titleText.fontStyle = FontStyle.Bold;
            titleText.color = new Color(0.60f, 0.40f, 1f, 1f);
            ...
            contentText.text = L10n.Get("dual_cast_upgrade_confirm", targetLevel);
