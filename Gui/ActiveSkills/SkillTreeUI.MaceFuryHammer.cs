    ...
    public partial class SkillTreeUI
    {
        ...
        private InvestResult CheckFuryHammerInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("mace_Step6_grandmaster") <= 0)
                return new InvestResult(false, L10n.Get("fury_hammer_prereq_required"));
        ...
        {
            if (node.Id != "mace_Step7_fury_hammer") return false;
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Mace_Config.FuryHammerRequiredPointsValue;
