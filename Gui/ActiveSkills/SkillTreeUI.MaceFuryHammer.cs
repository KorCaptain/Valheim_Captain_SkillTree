...
{
    // 분노의 망치(mace_Step7_fury_hammer) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckFuryHammerInvest(SkillTree.SkillNode node, int currentLevel)
        {
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasFuryHammerLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingFuryHammerItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleFuryHammerClick(SkillTree.SkillNode node)
        {
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("mace_Step7_fury_hammer");
            ...
            if (isAdmin || manager.HasFuryHammerLevelItems(targetLevel))
            {
                ...
                ShowFuryHammerUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowFuryHammerUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            borderImage.color = new Color(0.25f, 0.05f, 0.02f, 0.85f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.5f, 0.1f, 0.05f, 1f);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.6f, 0.1f, 0.05f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
