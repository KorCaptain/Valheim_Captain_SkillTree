...
{
    // 꿰뚫는 창(spear_Step5_penetrate) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckSpearPenetrateInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("spear_Step4_triple") <= 0)
                return new InvestResult(false, L10n.Get("spear_penetrate_prereq_required"));
            int targetLevel = currentLevel + 1;
            if (targetLevel > 7)
                return new InvestResult(false, L10n.Get("spear_penetrate_max_level"));
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasSpearPenetrateLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingSpearPenetrateItems(targetLevel);
                var msg = L10n.Get("spear_penetrate_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("spear_penetrate_missing_items", string.Join(", ", missing));
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleSpearPenetrateClick(SkillTree.SkillNode node)
        {
            if (node.Id != "spear_Step5_penetrate") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("spear_Step5_penetrate");
            int targetLevel = skillLevel + 1;
            if (targetLevel > 7)
            {
                tooltipUI.ShowWarning(L10n.Get("spear_penetrate_max_level"));
                return true;
            }
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Spear_Config.SpearPenetrateRequiredPointsValue;
            ...
            if (isAdmin || manager.HasSpearPenetrateLevelItems(targetLevel))
            {
                ...
                ShowSpearPenetrateUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
                var missing = manager.GetMissingSpearPenetrateItems(targetLevel);
                var msg = L10n.Get("spear_penetrate_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("spear_penetrate_missing_items", string.Join(", ", missing));
        ...
        // ─── 업그레이드 확인 다이얼로그 ───
        private void ShowSpearPenetrateUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            if (confirmDialog != null)
                DestroyImmediate(confirmDialog);

            confirmDialog = new GameObject("SpearPenetrateUpgradeDialog");
            ...
            titleText.text = L10n.Get("spear_penetrate_upgrade_title");
            ...
            contentText.text = L10n.Get("spear_penetrate_upgrade_confirm", targetLevel);
