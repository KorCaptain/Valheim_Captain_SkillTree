...
namespace CaptainSkillTree.Gui
{
    ...
    public partial class SkillTreeUI
    {
        ...
        private InvestResult CheckPenetrateInvest(SkillTree.SkillNode node, int currentLevel)
        {
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasPenetrateItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingPenetrateItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandlePenetrateClick(SkillTree.SkillNode node)
        {
            ...
            if (isAdmin || manager.HasPenetrateItems(targetLevel))
            {
                ...
                ShowPenetrateUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowPenetrateUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            borderImage.color = new Color(0.05f, 0.1f, 0.25f, 0.85f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.05f, 0.2f, 0.5f, 1f);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.05f, 0.25f, 0.55f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
