...
{
    // 관통 돌격(polearm_step5_king) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckPierceChargeInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0)
            {
                var mgr = SkillTree.SkillTreeManager.Instance;
                bool hasPrereq = mgr.GetSkillLevel("polearm_step3_ground") > 0
                              || mgr.GetSkillLevel("polearm_step4_moon") > 0
                              || mgr.GetSkillLevel("polearm_step1_suppress") > 0;
                if (!hasPrereq)
                    return new InvestResult(false, L10n.Get("pierce_charge_prereq_required"));
            }
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasPierceChargeLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingPierceChargeItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandlePierceChargeClick(SkillTree.SkillNode node)
        {
            if (node.Id != "polearm_step5_king") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("polearm_step5_king");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Polearm_Config.PolearmKingRequiredPointsValue;
            ...
            if (isAdmin || manager.HasPierceChargeLevelItems(targetLevel))
            {
                ...
                ShowPierceChargeUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
