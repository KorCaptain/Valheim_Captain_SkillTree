...
{
    // 폴암 휠윈드(polearm_step6_whirlwind) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        ...
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("polearm_step5_king") <= 0)
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasPolearmWhirlwindLevelItems(targetLevel))
            {
        ...
        {
            if (node.Id != "polearm_step6_whirlwind") return false;
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Polearm_Config.PolearmWhirlwindRequiredPointsValue;
            ...
            if (isAdmin || manager.HasPolearmWhirlwindLevelItems(targetLevel))
            {
