...
{
    // 폴암 회오리(polearm_step6_whirlwind) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckPolearmWhirlwindInvest(SkillTree.SkillNode node, int currentLevel)
        {
            int targetLevel = currentLevel + 1;
            if (targetLevel > 7)
                return new InvestResult(false, L10n.Get("polearm_whirlwind_max_level"));
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasWhirlwindItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingPolearmWhirlwindItems(targetLevel);
                var msg = L10n.Get("polearm_whirlwind_level_item_required", targetLevel);
                if (missing.Count > 0)
                    msg += "\n" + L10n.Get("polearm_whirlwind_missing_items", string.Join(", ", missing));
        ...
        // ─── 클릭 핸들러 ───
        private bool HandlePolearmWhirlwindClick(SkillTree.SkillNode node)
        {
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("polearm_step6_whirlwind");
            if (skillLevel < 0) return false;

            int targetLevel = skillLevel + 1;
            if (targetLevel > 7)
            {
                tooltipUI.ShowWarning(L10n.Get("polearm_whirlwind_max_level"));
                return true;
            }
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            ...
            if (isAdmin || manager.HasWhirlwindItems(targetLevel))
            {
                ...
                ShowPolearmWhirlwindUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowPolearmWhirlwindUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            confirmDialog = new GameObject("PolearmWhirlwindUpgradeDialog");
            ...
            borderImage.color = new Color(0.05f, 0.2f, 0.15f, 0.85f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.05f, 0.45f, 0.3f, 1f);
            ...
            titleText.text = L10n.Get("polearm_whirlwind_upgrade_title");
            ...
            contentText.text = L10n.Get("polearm_whirlwind_upgrade_confirm", targetLevel);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.05f, 0.45f, 0.30f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
