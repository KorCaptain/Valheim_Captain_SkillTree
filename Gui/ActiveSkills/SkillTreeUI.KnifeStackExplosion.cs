...
{
    // 약점폭발(knife_step10_stack_explosion) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckStackExplosionInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("knife_step8_assassination") <= 0)
                return new InvestResult(false, L10n.Get("stack_explosion_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasStackExplosionLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingStackExplosionItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleStackExplosionClick(SkillTree.SkillNode node)
        {
            if (node.Id != "knife_step10_stack_explosion") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("knife_step10_stack_explosion");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Knife_Config.KnifeStackExplosionRequiredPointsValue;
            ...
            if (isAdmin || manager.HasStackExplosionLevelItems(targetLevel))
            {
                ...
                ShowStackExplosionUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowStackExplosionUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            darkBgImage.color = new Color(0.10f, 0.06f, 0.04f, 0.97f);
            ...
            titleBar.AddComponent<Image>().color = new Color(0.20f, 0.09f, 0.04f, 1f);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.65f, 0.25f, 0.05f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
