...
{
    // 폭발화살(bow_Step6_critboost) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        ...
        private InvestResult CheckExplosiveArrowInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("bow_Step5_master") <= 0)
                return new InvestResult(false, L10n.Get("explosive_arrow_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasExplosiveArrowLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingExplosiveArrowItems(targetLevel);
        ...
        private bool HandleExplosiveArrowClick(SkillTree.SkillNode node)
        {
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Bow_Config.BowExplosiveArrowRequiredPointsValue;
            ...
            if (isAdmin || manager.HasExplosiveArrowLevelItems(targetLevel))
            {
                ...
                ShowExplosiveArrowUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
