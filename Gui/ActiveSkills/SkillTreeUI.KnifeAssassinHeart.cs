...
{
    // 암살자의 심장(knife_step9_assassin_heart) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckAssassinHeartInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("knife_step8_assassination") <= 0)
                return new InvestResult(false, L10n.Get("assassin_heart_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasAssassinHeartLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingAssassinHeartItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleAssassinHeartClick(SkillTree.SkillNode node)
        {
            if (node.Id != "knife_step9_assassin_heart") return false;
            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("knife_step9_assassin_heart");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Knife_Config.KnifeAssassinHeartRequiredPointsValue;
            ...
            if (isAdmin || manager.HasAssassinHeartLevelItems(targetLevel))
            {
                ...
                ShowAssassinHeartUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
