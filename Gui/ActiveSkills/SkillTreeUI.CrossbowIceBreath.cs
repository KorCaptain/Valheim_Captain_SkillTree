...
{
    // 빙결폭발탄(crossbow_ice_breath) UI — 업그레이드 다이얼로그, 핸들러, 투자 검증
    public partial class SkillTreeUI
    {
        // ─── CanInvestWithMessage 분기 ───
        private InvestResult CheckIceBreathInvest(SkillTree.SkillNode node, int currentLevel)
        {
            if (currentLevel == 0 && SkillTree.SkillTreeManager.Instance.GetSkillLevel("crossbow_Step6_expert") <= 0)
                return new InvestResult(false, L10n.Get("icebreath_prereq_required"));
            ...
            if (!isAdmin && !SkillTree.SkillTreeManager.Instance.HasIceBreathLevelItems(targetLevel))
            {
                var missing = SkillTree.SkillTreeManager.Instance.GetMissingIceBreathItems(targetLevel);
        ...
        // ─── 클릭 핸들러 ───
        private bool HandleIceBreathClick(SkillTree.SkillNode node)
        {
            if (node.Id != "crossbow_ice_breath") return false;

            var manager = SkillTree.SkillTreeManager.Instance;
            int skillLevel = manager.GetSkillLevel("crossbow_ice_breath");
            ...
            bool isAdmin = CaptainSkillTree.MMO_System.CaptainLevelConfig.IsAdminModeActive();
            int requiredPts = SkillTree.Crossbow_Config.CrossbowIceBreathRequiredPointsValue;
            ...
            if (isAdmin || manager.HasIceBreathLevelItems(targetLevel))
            {
                ...
                ShowIceBreathUpgradeConfirmDialog(targetLevel, nodeRect);
            }
            else
            {
        ...
        private void ShowIceBreathUpgradeConfirmDialog(int targetLevel, RectTransform nodeRect)
        {
            ...
            titleBar.AddComponent<Image>().color = new Color(0.05f, 0.10f, 0.30f, 1f);
            ...
            confirmButton = CreateLuxuryButton("ConfirmButton", L10n.Get("ui_confirm"),
                new Vector2(-68f, -102f), new Color(0.10f, 0.20f, 0.60f, 1f), darkBg.transform, () =>
                {
                    PlayConfirmSound();
